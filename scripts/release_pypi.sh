...
if [[ "$VERSION_STR" =~ $SEMVER_REGEX ]]; then
	echo "Releasing vtsserving version v$VERSION_STR:"
else
...
# Currently only VtsServing maintainer has permission to create new pypi
# releases
...
if git rev-parse "$tag_name" >/dev/null 2>&1; then
	...
	git tag -a "$tag_name" -m "Tag generated by VtsServing/script/release.sh, version: $VERSION_STR"
...
echo "Done releasing VtsServing version:$VERSION_STR"
