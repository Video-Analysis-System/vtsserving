...
docker buildx build --platform=linux/arm64,linux/amd64 -t bentoml/quickstart:$VTSSERVING_VERSION -t bentoml/quickstart:latest --pull -o type=image,push=True -f- . <<EOF
...
RUN pip install -U pip "bentoml[grpc]==${VTSSERVING_VERSION}" && pip install -r ./requirements.txt
