...
def validate_container_tag(
    ...
    if not tag:
        ...
        raise VtsServingException(f"Invalid tag type. Got {type(tag)}")


class VtsServingCommandGroup(click.Group):
    """
    Click command class customized for VtsServing CLI, allow specifying a default
    ...
    .. code-block:: python
        ...
        @click.group(cls=VtsServingCommandGroup)
        def cli(): ...
    ...
    @staticmethod
    def vtsserving_common_params(func: F[P]) -> WrappedCLI[bool, bool]:
        # NOTE: update NUMBER_OF_COMMON_PARAMS when adding option.
        from vtsserving._internal.log import configure_logging
        from vtsserving._internal.configuration import DEBUG_ENV_VAR
        from vtsserving._internal.configuration import QUIET_ENV_VAR
        from vtsserving._internal.configuration import set_debug_mode
        from vtsserving._internal.configuration import set_quiet_mode
        from vtsserving._internal.utils.analytics import VTSSERVING_DO_NOT_TRACK
    ...
    @staticmethod
    def vtsserving_track_usage(
        func: F[P] | WrappedCLI[bool, bool],
        cmd_group: click.Group,
        **kwargs: t.Any,
    ) -> WrappedCLI[bool]:
        from vtsserving._internal.utils.analytics import track
        from vtsserving._internal.utils.analytics import CliEvent
        from vtsserving._internal.utils.analytics import cli_events_map
        from vtsserving._internal.utils.analytics import VTSSERVING_DO_NOT_TRACK
        ...
        def wrapper(do_not_track: bool, *args: P.args, **kwargs: P.kwargs) -> t.Any:
            ...
            def get_tracking_event(return_value: t.Any) -> CliEvent:
                ...
                # since we know the cmd_group.name to be VtsServingCommandGroup
    ...
    def raise_click_exception(
        ...
        from vtsserving.exceptions import VtsServingException
        from vtsserving._internal.configuration import get_debug_mode
        ...
        def wrapper(*args: P.args, **kwargs: P.kwargs) -> t.Any:
            try:
                return func(*args, **kwargs)
            except VtsServingException as err:
    ...
    def __init__(self, *args: t.Any, **kwargs: t.Any) -> None:
        super(VtsServingCommandGroup, self).__init__(*args, **kwargs)
    ...
    def command(self, *args: t.Any, **kwargs: t.Any) -> t.Callable[[F[P]], Command]:
        ...
        def wrapper(func: F[P]) -> Command:
            ...
            options = VtsServingCommandGroup.vtsserving_common_params(func)
            # Send tracking events before command finish.
            usage = VtsServingCommandGroup.vtsserving_track_usage(options, self, **kwargs)
            # If VtsServingException raise ClickException instead before exit.
            wrapped = VtsServingCommandGroup.raise_click_exception(usage, self, **kwargs)
            ...
            cmd = super(VtsServingCommandGroup, self).command(*args, **kwargs)(wrapped)
    ...
    def get_command(self, ctx: Context, cmd_name: str) -> Command | None:
        ...
        return super(VtsServingCommandGroup, self).get_command(ctx, cmd_name)
    ...
    def resolve_command(
        ...
        try:
            return super(VtsServingCommandGroup, self).resolve_command(ctx, args)
...
def is_valid_vts_tag(value: str) -> bool:
    try:
...
def is_valid_vts_name(value: str) -> bool:
