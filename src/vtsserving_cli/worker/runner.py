...
def main(
    ...
    assert (
        "vtsserving" not in sys.modules
    ), "vtsserving should not be imported before setting up the environment, otherwise some of the environment may not take effect"
    ...
    from vtsserving._internal.server.runner_app import RunnerAppFactory

    service = load(vts_identifier, working_dir=working_dir, standalone_load=True)

    # setup context
    if service.tag is None:
        component_context.vts_name = service.name
        component_context.vts_version = "not available"
    else:
        component_context.vts_name = service.tag.name
        component_context.vts_version = service.tag.version or "not available"
