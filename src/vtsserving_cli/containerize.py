...
if TYPE_CHECKING:
    ...
    from vtsserving._internal.container import DefaultBuilder
...
def add_containerize_command(cli: Group) -> None:
    ...
    from vtsserving import container
    from vtsserving_cli.utils import kwargs_transformers
    from vtsserving_cli.utils import validate_container_tag
    from vtsserving.exceptions import VtsServingException
    from vtsserving._internal.container import FEATURES
    from vtsserving._internal.container import enable_buildkit
    from vtsserving._internal.container import REGISTERED_BACKENDS
    from vtsserving._internal.container import determine_container_tag
    ...
    @cli.command()
    @click.argument("vts_tag", type=click.STRING, metavar="VTS:TAG")
    @click.option(
        "-t",
        "--image-tag",
        "--docker-image-tag",
        metavar="NAME:TAG",
        help="Name and optionally a tag (format: ``name:tag``) for building container, defaults to the vts tag.",
        required=False,
    ...
    @click.option(
        ...
        help="Enable additional VtsServing features. Available features are: {}.".format(
    ...
    def containerize(  # type: ignore
        ...
        **kwargs: t.Any,  # pylint: disable=unused-argument
        ...
        .. code-block:: bash
           ...
           $ vtsserving containerize iris_classifier:latest -t repo-address.com:username/iris
        ...
        \b
        ``vtsserving containerize`` also leverage BuildKit features such as multi-node
        ...
        .. code-block:: bash
            ...
            $ DOCKER_BUILDKIT=0 vtsserving containerize iris_classifier:latest

        \b
        ``vtsserving containerize`` is backend-agnostic, meaning that it can use any OCI-compliant builder to build the container image.
        ...
        if not container.health(backend):
            raise VtsServingException("Failed to use backend %s." % backend)
        ...
        if result is not None:
            tags = determine_container_tag(vts_tag, image_tag=image_tag)
