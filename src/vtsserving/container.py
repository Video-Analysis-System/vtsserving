...
def build(
    ...
    **kwargs: t.Any,
    ...
    """
    Build any given VtsServing into a OCI-compliant image.

    .. code-block:: python

        import vtsserving

        vts = vtsserving.get("pytorch_vgg:latest")
    ...
    Args:
        ...
        backend: The backend to use for building the image. Current supported builder backends
                 ...
                 .. note::
                     ...
                     The reason for this is that ``buildx`` used to be the default behaviour of ``vtsserving containerize``.
        image_tag: Optional additional image tag to apply to the built image.
        features: Optional features to include in the container file. See https://docs.vtsserving.org/en/latest/concepts/vts.html#python-packages
                  for additional VtsServing features.
    """
    ...
    if "tag" not in kwargs:
        kwargs["tag"] = determine_container_tag(vts_tag, image_tag=image_tag)
    vts = _vts_store.get(vts_tag)
    ...
    if platform is not None:
        ...
        if not psutil.LINUX and "linux/amd64" not in passed_platform:
            logger.warning(
                'Current platform is set to "--opt platform=%s". To avoid issue, we recommend you to build the container with x86_64 (amd64): "vtsserving containerize %s --opt platform=linux/amd64"',
                passed_platform,
                str(vts.tag),
            )
    return _internal_build(vts_tag, backend, features=features, **kwargs)
...
def get_containerfile(
    ...
    Args:
        ...
        output_path: Optional output path to write the final container file to.
                     ...
                     will be ``output_path + os.sep + "<vts_tag>.dockerfile"``.
        ...
        if output_path is not None:
            if os.path.isdir(output_path):
                output_path = output_path = os.path.join(
                    output_path, f"{vts.tag.path().replace(os.sep,'_')}.dockerfile"
                )
            logger.info(
                "Writting Containerfile for '%s' to '%s'.", vts.tag, output_path
            )
