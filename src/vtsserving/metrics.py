...
# While this is less than optimal, we will do this since 'vtsserving.metrics'
# is a public API.
...
.. code-block:: python
    ...
    from vtsserving.metrics import Histogram
...
If you need to go down, uses :func:`vtsserving.metrics.Gauge` instead.

A quick example of a Counter:

.. code-block:: python

    from vtsserving.metrics import Counter
...
Similar to :meth:`vtsserving.metrics.Histogram`, ``time()`` can also be used as a decorator or context manager.
...
One can think of Gauge as a :meth:`vtsserving.metrics.Counter` that can go up and down.
...
.. code-block:: python
    ...
    from vtsserving.metrics import Gauge
...
def __getattr__(item: t.Any):
    if item in _NOT_SUPPORTED:
        raise NotImplementedError(
            f"{item} is not supported when using '{__name__}'. See https://docs.vtsserving.org/en/latest/reference/metrics.html."
        )
    # This is the entrypoint for all vtsserving.metrics.*
...
class _LazyMetric:
    ...
    def __call__(self, *args: t.Any, **kwargs: t.Any) -> t.Any:
        ...
        if "registry" in kwargs:
            raise ValueError(
                f"'registry' should not be passed when using '{__name__}.{self._attr}'. See https://docs.vtsserving.org/en/latest/reference/metrics.html."
            )
        warn_experimental("%s.%s" % (__name__, self._attr))
        self._args = args
        self._kwargs = kwargs
        if self._attr in _INTERNAL_FN_IMPL:
            # first-class function implementation from VtsServing Prometheus client.
