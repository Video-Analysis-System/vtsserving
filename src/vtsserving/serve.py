...
SCRIPT_DEV_API_SERVER = "vtsserving_cli.worker.http_dev_api_server"
SCRIPT_GRPC_API_SERVER = "vtsserving_cli.worker.grpc_api_server"
SCRIPT_GRPC_DEV_API_SERVER = "vtsserving_cli.worker.grpc_dev_api_server"
SCRIPT_GRPC_PROMETHEUS_SERVER = "vtsserving_cli.worker.grpc_prometheus_server"

API_SERVER_NAME = "_vts_api_server"
PROMETHEUS_SERVER_NAME = "_prometheus_server"


@inject
def ensure_prometheus_dir(
    directory: str = Provide[VtsServingContainer.prometheus_multiproc_dir],
    clean: bool = True,
    ...
    try:
        ...
        VtsServingContainer.prometheus_multiproc_dir.set(alternative)
        return alternative
    except Exception as e:  # pylint: disable=broad-except
        raise RuntimeError(
...
def serve_http_development(
    vts_identifier: str,
    working_dir: str,
    port: int = Provide[VtsServingContainer.http.port],
    host: str = Provide[VtsServingContainer.http.host],
    backlog: int = Provide[VtsServingContainer.api_server_config.backlog],
    vtsserving_home: str = Provide[VtsServingContainer.vtsserving_home],
    ...
    svc = load(vts_identifier, working_dir=working_dir)
    ...
    if reload:
        ...
        logger.debug(
            "--reload is passed. VtsServing will watch file changes based on 'vtsfile.yaml' and '.vtsignore' respectively."
        )
        ...
        plugins = [
            # reloader plugin
            {
                "use": "vtsserving._internal.utils.circus.watchfilesplugin.ServiceReloaderPlugin",
                "working_dir": working_dir,
