...
def list(  # pylint: disable=redefined-builtin
    ...
    *,
    _model_store: "ModelStore" = Provide[VtsServingContainer.model_store],
) -> t.List["Model"]:
...
def import_model(
    ...
    *,
    ...
    """
    Import a vts model exported with :code:`vtsserving.models.export_model`. To import a model saved
    with a framework, see the :code:`save` function under the relevant framework, e.g.
    :code:`vtsserving.sklearn.save`.

    Examples:

    .. code-block:: python

        # imports 'my_model' from '/path/to/folder/my_model.vtsmodel'
        ...
        vtsserving.models.import_model('/path/to/folder/my_model.tar.gz')
        # treats 'my_model.ext' as a gzipped tarfile
        vtsserving.models.import_model('/path/to/folder/my_model.ext', 'gz')

        # imports 'my_model', which is stored as an uncompressed folder, from '/path/to/folder/my_model/'
        vtsserving.models.import_model('/path/to/folder/my_model', 'folder')

        # imports 'my_model' from the S3 bucket 'my_bucket', path 'folder/my_model.vtsmodel'
        ...
        vtsserving.models.import_model('my_bucket/folder/my_model.vtsmodel', protocol='s3')
        vtsserving.models.import_model('my_bucket', protocol='s3', subpath='folder/my_model.vtsmodel')
        vtsserving.models.import_model('my_bucket', protocol='s3', subpath='folder/my_model.vtsmodel',
    ...
    Args:
        ...
        path: can be one of two things:
            ...
            * an `FS URL <https://docs.pyfilesystem.org/en/latest/openers.html>`_, for example
                :code:`'s3://my_bucket/folder/my_model.vtsmodel'`
...
def export_model(
    ...
    *,
    ...
    """
    Export a VtsServing model.

    Examples:

    .. code-block:: python

        # exports 'my_model' to '/path/to/folder/my_model-version.vtsmodel' in VtsServing's default format
        vtsserving.models.export_model('my_model:latest', '/path/to/folder')
        ...
        # exports 'my_model' to '/path/to/folder/my_model.vtsmodel' in VtsServing's default format
        vtsserving.models.export_model('my_model:latest', '/path/to/folder/my_model')
        vtsserving.models.export_model('my_model:latest', '/path/to/folder/my_model.vtsmodel')

        # exports 'my_model' to '/path/to/folder/my_model.tar.gz in gzip format
        # currently supported formats are tar.gz ('gz'), tar.xz ('xz'), tar.bz2 ('bz2'), and zip
        vtsserving.models.export_model('my_model:latest', '/path/to/folder/my_model.tar.gz')
        vtsserving.models.export_model('my_model:latest', '/path/to/folder/my_model.tar.gz', 'gz')

        # exports 'my_model' to '/path/to/folder/my_model/ as a folder
        vtsserving.models.export_model('my_model:latest', '/path/to/folder/my_model', 'folder')

        # exports 'my_model' to the S3 bucket 'my_bucket' as 'folder/my_model-version.vtsmodel'
        vtsserving.models.export_model('my_model:latest', 's3://my_bucket/folder')
        vtsserving.models.export_model('my_model:latest', 'my_bucket/folder', protocol='s3')
        vtsserving.models.export_model('my_model:latest', 'my_bucket', protocol='s3', subpath='folder')
        vtsserving.models.export_model('my_model:latest', 'my_bucket', protocol='s3', subpath='folder',
                                    user='<AWS access key>', passwd='<AWS secret key>',
                                    params={'acl': 'public-read', 'cache-control': 'max-age=2592000,public'})
...
def push(
    ...
    *,
    ...
    if not model_obj:
        raise VtsServingException(f"Model {tag} not found in local store")
