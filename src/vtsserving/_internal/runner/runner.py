...
class Runner:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            models: list of required vts models
        ...
        try:
            ...
            raise ValueError(
                f"Runner name '{name}' is not valid; it must be a valid VtsServing Tag name."
            ) from e

        runners_config = VtsServingContainer.config.runners.get()
        ...
        if models is None:
            ...
            if not all(isinstance(model, Model) for model in models):
                raise ValueError(
                    f"models must be a list of 'vtsserving.Model'. Got { {type(model) for model in models if isinstance(model, Model)} } instead."
                )
        ...
        if runnable_class.vtsserving_runnable_methods__ is None:
            raise ValueError(
                f"Runnable class '{runnable_class.__name__}' has no methods!"
            )

        for method_name, method in runnable_class.vtsserving_runnable_methods__.items():
    ...
    async def runner_handle_is_ready(
        self,
        timeout: int = Provide[VtsServingContainer.api_server_config.runner_probe.timeout],
    ) -> bool:
