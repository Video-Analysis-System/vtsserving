...
class RemoteRunnerClient(RunnerHandle):
    ...
    def _remote_runner_server_map(self) -> dict[str, str]:
        return VtsServingContainer.remote_runner_mapping.get()

    @property
    def runner_timeout(self) -> int:
        "return the configured timeout for this runner."
        runner_cfg = VtsServingContainer.runners_config.get()
    ...
    async def async_run_method(
        self,
        __vtsserving_method: RunnerMethod[t.Any, P, R],
        *args: P.args,
        **kwargs: P.kwargs,
    ) -> R | tuple[R, ...]:
        import aiohttp
        ...
        try:
            ...
            if os.getenv("VTSSERVING_RETRY_RUNNER_REQUESTS", "").lower() == "true":
                try:
        ...
        try:
            ...
            assert content_type.lower().startswith("application/vnd.vtsserving.")
        ...
        container = content_type.strip("application/vnd.vtsserving.")
