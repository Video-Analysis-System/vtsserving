...
class Store(ABC, t.Generic[Item]):
    ...
    def _recreate_latest(self, tag: Tag):
        try:
            ...
            raise NotFound(
                f"no {self._item_type.get_typename()}s with name '{tag.name}' exist in VtsServing store {self._fs}"
            )
    ...
    def get(self, tag: t.Union[Tag, str]) -> Item:
        """
        store.get("my_vts")
        store.get("my_vts:v1.0.0")
        store.get(Tag("my_vts", "latest"))
        """
        _tag = Tag.from_taglike(tag)
        ...
        if counts == 0:
            raise NotFound(
                f"{self._item_type.get_typename()} '{tag}' is not found in VtsServing store {self._fs}"
            )
        elif counts == 1:
