...
def validate_tag_str(value: str):
    ...
    if errors:
        ...
        raise ValueError(
            f"{value} is not a valid VtsServing tag: " + ", and ".join(errors)
        )
...
vtsserving_cattr.register_structure_hook(Tag, lambda d, _: Tag.from_taglike(d))  # type: ignore[misc]
vtsserving_cattr.register_unstructure_hook(Tag, str)
