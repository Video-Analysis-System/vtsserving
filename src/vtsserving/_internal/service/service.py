...
from ..vts.vts import get_default_svc_readme
...
class Service:
    ...
    and the core building block in VtsServing where users define the service runtime
    architecture and model serving logic.

    A VtsServing service is defined via instantiate this Service class. When creating a
        ...
        *,
        ...
        if runners is not None:
            ...
            for r in runners:
                assert isinstance(
                    r, Runner
                ), f'Service runners list can only contain vtsserving.Runner instances, type "{type(r)}" found.'
        ...
        if models is not None:
            for model in models:
                assert isinstance(
                    model, Model
                ), f'Service models list can only contain vtsserving.Model instances, type "{type(model)}" found.'
    ...
    def __str__(self):
        if self.vts:
            return f'vtsserving.Service(tag="{self.tag}", ' f'path="{self.vts.path}")'
        elif self._import_str and self._working_dir:
            return (
                f'vtsserving.Service(name="{self.name}", '
