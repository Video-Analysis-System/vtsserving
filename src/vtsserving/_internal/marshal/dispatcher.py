...
class CorkDispatcher:
    ...
    async def controller(self):
        ...
        if self.optimizer.o_a + self.optimizer.o_b >= self.max_latency_in_ms:
            logger.warning(
                "VtsServing has detected that a service has a max latency that is likely too low for serving. If many 429 errors are encountered, try raising the 'runner.max_latency' in your VtsServing configuration YAML file."
            )
