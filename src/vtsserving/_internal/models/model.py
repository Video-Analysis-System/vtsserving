...
class Model(StoreItem):
        ...
        *,
        ...
        if not _internal:
            raise VtsServingException(
                "Model cannot be instantiated directly directly; use vtsserving.<framework>.save or vtsserving.models.get instead"
            )
    ...
    def create(
        ...
        *,
        ...
        Args:
            ...
            context: Environment context managed by VtsServing for loading model,
        ...
        model_fs = fs.open_fs(f"temp://vtsserving_model_{tag.name}")
    ...
    @inject
    def save(
        self, model_store: ModelStore = Provide[VtsServingContainer.model_store]
    ) -> Model:
    ...
    def from_fs(cls: t.Type[Model], item_fs: FS) -> Model:
        try:
            ...
            raise VtsServingException(
                f"Failed to load vts model because it does not contain a '{MODEL_YAML_FILENAME}'"
            )
        ...
        try:
            ...
            raise VtsServingException(f"Failed to load {self!s}: {e}") from None
...
class ModelContext:
    ...
    def from_dict(data: dict[str, str | dict[str, str]] | ModelContext) -> ModelContext:
        if isinstance(data, ModelContext):
            return data
        return vtsserving_cattr.structure(data, ModelContext)
...
class ModelSignature:
    ...
    This information is used when creating VtsServing runners for this model.
    ...
    Fields:
        batchable:
            Whether multiple API calls to this predict method should be batched by the VtsServing
            runner.
        batch_dim:
            ...
            Example: .. code-block:: python
                ...
                dimension 0 and the other on dimension 1: vtsserving.pytorch.save_model("demo0",
                model_0, signatures={"predict": {"batchable": True, "batch_dim": 0}})
                vtsserving.pytorch.save_model("demo1", model_1, signatures={"predict": {"batchable":
                True, "batch_dim": 1}})
                ...
                vtsserving.pytorch.get("demo0:latest").to_runner() runner0.init_local()
                runner0.predict.run(np.array([[1, 2], [3, 4]])) runner0.predict.run(np.array([[5,
                6]]))
                # if the following calls are batched, the input to the actual predict method on the
                # model.predict would be [[1, 2, 5], [3, 4, 6]] runner1 =
                vtsserving.pytorch.get("demo1:latest").to_runner() runner1.init_local()
    ...
    def from_dict(cls, data: ModelSignatureDict) -> ModelSignature:
        if "batch_dim" in data and isinstance(data["batch_dim"], int):
            ...
            formated_data = data
        return vtsserving_cattr.structure(formated_data, cls)
...
class ModelInfo:
    ...
    def to_dict(self) -> t.Dict[str, t.Any]:
        return vtsserving_cattr.unstructure(self)  # type: ignore (incomplete cattr types)
    ...
    def from_yaml_file(cls, stream: t.IO[t.Any]) -> ModelInfo:
        ...
        if not isinstance(yaml_content, dict):
            raise VtsServingException(f"malformed {MODEL_YAML_FILENAME}")
        ...
        if "vtsserving_version" in yaml_content:
            del yaml_content["vtsserving_version"]
        ...
        try:
            model_info = vtsserving_cattr.structure(yaml_content, cls)
        except TypeError as e:  # pragma: no cover - simple error handling
            raise VtsServingException(f"unexpected field in {MODEL_YAML_FILENAME}: {e}")
        return model_info

    def validate(self):
        # Validate model.yml file schema, content, vtsserving version, etc
...
vtsserving_cattr.register_unstructure_hook_func(
