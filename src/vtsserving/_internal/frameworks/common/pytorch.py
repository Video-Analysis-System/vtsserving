...
try:
    ...
    raise MissingDependencyException(
        "'torch' is required in order to use module 'vtsserving.pytorch', 'vtsserving.torchscript' or 'vtsserving.pytorch_lightning'. Install torch with 'pip install torch'. For more information, refer to https://pytorch.org/get-started/locally/"
    )
...
class PytorchModelRunnable(vtsserving.Runnable):
    ...
    def __init__(
        ...
        if torch.cuda.is_available():
            ...
            self.device_id = "cpu"
        self.model: ModelType = loader(vts_model, device_id=self.device_id)
...
class PyTorchTensorContainer(DataContainer[torch.Tensor, torch.Tensor]):
    ...
    def to_payload(  # pylint: disable=arguments-differ
        ...
        batch: torch.Tensor,
        batch_dim: int = 0,
        plasma_db: "ext.PlasmaClient" | None = Provide[VtsServingContainer.plasma_db],
    ) -> Payload:
