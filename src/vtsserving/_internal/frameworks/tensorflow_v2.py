...
from vtsserving import Runnable
...
try:
    ...
    raise MissingDependencyException(
        "'tensorflow' is required in order to use module 'vtsserving.tensorflow', install tensorflow with 'pip install tensorflow'. For more information, refer to https://www.tensorflow.org/install"
    )

MODULE_NAME = "vtsserving.tensorflow"
API_VERSION = "v1"
...
def load_model(
    ...
    """
    Load a tensorflow model from VtsServing local modelstore with given name.
    ...
    Returns:
        :obj:`SavedModel`: an instance of :obj:`SavedModel` format from VtsServing modelstore.
    ...
    .. code-block:: python
        ...
        model = vtsserving.tensorflow.load_model("my_tensorflow_model")
    ...
    with tf.device(device_name):  # type: ignore (tf.device is a context manager)
        tf_model: tf_ext.AutoTrackable = tf.saved_model.load(vts_model.path)
        return tf_model
...
def save_model(
    ...
    *,
    ...
    Returns:
        ...
        the user-defined model's name, and a generated `version` by VtsServing.
    ...
    .. code-block:: python
        ...
        # then save the given model to VtsServing modelstore:
        model = NativeModel()
        vts_model = vtsserving.tensorflow.save_model("native_toy", model)

    .. note::

       :code:`vtsserving.tensorflow.save_model` API also support saving `RaggedTensor <https://www.tensorflow.org/guide/ragged_tensor>`_ model and Keras model. If you choose to save a Keras model
       with :code:`vtsserving.tensorflow.save_model`, then the model will be saved under a :obj:`SavedModel` format instead of :obj:`.h5`.
...
def get_runnable(
    ...
    partial_kwargs: t.Dict[str, t.Any] = vts_model.info.options.partial_kwargs
    ...
    class TensorflowRunnable(Runnable):
        ...
        def __init__(self):
            ...
            self.model = load_model(vts_model, device_name=self.device_name)
