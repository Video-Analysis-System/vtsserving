...
TF_FUNCTION_WARNING = "Due to TensorFlow's internal mechanism, only methods wrapped under '@tf.function' decorator and the Keras default function '__call__(inputs, training=False)' can be restored after a save & load. You can test the restored model object via 'vtsserving.tensorflow.load_model(tag)'."

KERAS_MODEL_WARNING = "VtsServing detected that %s is being used to pack a Keras API based model. In order to get optimal serving performance, we recommend to wrap your keras model 'call()' methods with '@tf.function' decorator."
...
def hook_loaded_model(
    ...
    """
    deprecated: vtsserving now requires signatures before saving a tf model
...
def check_tensor_spec(
    ...
    if tensor_spec is None:
        raise VtsServingException("`tensor` should not be None")
...
def normalize_spec(value: t.Any) -> "tf_ext.TypeSpec":
    ...
    raise VtsServingException(f"Unknown type for tensor spec, got{type(value)}.")
