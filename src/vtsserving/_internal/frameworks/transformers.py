...
try:
    ...
    raise MissingDependencyException(
        "'transformers' is required in order to use module 'vtsserving.transformers'. Install transformers with 'pip install transformers'."
    )
...
MODULE_NAME = "vtsserving.transformers"
API_VERSION = "v1"
...
def get(tag_like: str | Tag) -> Model:
    ...
    .. code-block:: python
       ...
       model = vtsserving.transformers.get("my_pipeline:latest")
    """
...
def load_model(
    ...
    **kwargs: t.Any,
    ...
    """
    Load the Transformers model from VtsServing local modelstore with given name.
    ...
    .. code-block:: python
        ...
        import vtsserving
        pipeline = vtsserving.transformers.load_model('my_model:latest')
    """
    ...
    if TYPE_CHECKING:
        options = t.cast(TransformersOptions, vts_model.info.options)
    else:
        options = vts_model.info.options

    task: str = vts_model.info.options.task  # type: ignore
    if task not in SUPPORTED_TASKS:
        ...
        with open(vts_model.path_of(PIPELINE_PICKLE_NAME), "rb") as f:
    ...
    return transformers.pipeline(task=task, model=vts_model.path, **extra_kwargs)
    ...
    *,
    ...
    Args:
        ...
        task_definition: Task definition for the Transformers custom pipeline. The definition is a dictionary
                        ...
                        See module `src/transformers/pipelines/__init__.py <https://github.com/huggingface/transformers/blob/main/src/transformers/pipelines/__init__.py#L129>`_ for more details.
        signatures: Methods to expose for running inference on the target model. Signatures are used for creating :obj:`~vtsserving.Runner` instances when serving model with :obj:`~vtsserving.Service`
    ...
    .. code-block:: python
        ...
        vts_model = vtsserving.transformers.save_model("text-generation-pipeline", generator)
    """  # noqa
    _check_flax_supported()
    if not isinstance(
        pipeline,
        ...
        raise VtsServingException(
            ...
            vtsserving.transformers.save_model("text-generation-pipeline", generator)
            ```
            """
        )
    ...
    if task_name is not None and task_definition is not None:
        ...
        with vtsserving.models.create(
            ...
            pipeline.save_pretrained(vts_model.path)

            with open(vts_model.path_of(PIPELINE_PICKLE_NAME), "wb") as f:
...
def get_runnable(
    vts_model: vtsserving.Model,
) -> t.Type[vtsserving.Runnable]:
    """
    Private API: use :obj:`~vtsserving.Model.to_runnable` instead.
    """

    class TransformersRunnable(vtsserving.Runnable):
        ...
        def __init__(self):
            ...
            self.pipeline = load_model(vts_model, **kwargs)
