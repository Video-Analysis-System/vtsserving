...
from .build_dev_vtsserving_whl import build_vtsserving_editable_wheel
...
def _convert_python_version(py_version: str | None) -> str | None:
    ...
    if target_python_version != py_version:
        logger.warning(
            "VtsServing will install the latest 'python%s' instead of the specified 'python%s'. To use the exact python version, use a custom docker base image. See https://docs.vtsserving.org/en/latest/concepts/vts.html#custom-base-image-advanced",
...
def _convert_env(
    ...
    if isinstance(env, str):
        ...
        raise VtsServingException(f"'{env}' is not a valid '.env' file path")
...
class DockerOptions:
    ...
    # For validating user defined vtsfile.yaml.
    __forbid_extra_keys__ = True
    # always omit config values in case of default values got changed in future VtsServing releases
    ...
    def write_to_vts(self, vts_fs: FS, build_ctx: str, conda: CondaOptions) -> None:
        docker_folder = fs.path.combine("env", "docker")
        vts_fs.makedirs(docker_folder, recreate=True)
        ...
        # By default, VtsServing containerization will use BuildKit. To opt-out specify DOCKER_BUILDKIT=0
        vts_fs.writetext(
            dockerfile_path,
            generate_containerfile(
                self, build_ctx, conda=conda, vts_fs=vts_fs, enable_buildkit=False
            ),
        )
        ...
        if self.setup_script:
            ...
            copy_file_to_fs_folder(
                setup_script, vts_fs, docker_folder, "setup_script"
            )
...
class CondaOptions:
    ...
    __forbid_extra_keys__ = True
    # no need to omit since VtsServing has already handled the default values.
    __omit_if_default__ = False
    ...
    def write_to_vts(self, vts_fs: FS, build_ctx: str) -> None:
        if self.is_empty():
            return

        conda_folder = fs.path.join("env", "conda")
        vts_fs.makedirs(conda_folder, recreate=True)
    ...
    def get_python_version(self, vts_fs: FS) -> str | None:
        # Get the python version from given environment.yml file

        environment_yml = vts_fs.getsyspath(
            fs.path.join(
                "env",
                "conda",
...
class PythonOptions:
    ...
    def write_to_vts(self, vts_fs: FS, build_ctx: str) -> None:
        ...
        vts_fs.makedirs(py_folder, recreate=True)

        # Save the python version of current build environment
        with vts_fs.open(fs.path.join(py_folder, "version.txt"), "w") as f:
            f.write(f"{version_info.major}.{version_info.minor}.{version_info.micro}")

        # Build VtsServing whl from local source if VTSSERVING_BUNDLE_LOCAL_BUILD=True
        build_vtsserving_editable_wheel(vts_fs.getsyspath(wheels_folder))

        # Move over required wheel files
        if self.wheels is not None:
            vts_fs.makedirs(wheels_folder, recreate=True)
            for whl_file in self.wheels:  # pylint: disable=not-an-iterable
                whl_file = resolve_user_filepath(whl_file, build_ctx)
                copy_file_to_fs_folder(whl_file, vts_fs, wheels_folder)
        ...
        with vts_fs.open(fs.path.combine(py_folder, "install.sh"), "w") as f:
...
# VtsServing by default generates two requirement files:
...
# Install the VtsServing from PyPI if it's not already installed
if python3 -c "import vtsserving" &> /dev/null; then
    existing_vtsserving_version=$(python3 -c "import vtsserving; print(vtsserving.__version__)")
    if [ "$existing_vtsserving_version" != "$VTSSERVING_VERSION" ]; then
        echo "WARNING: using VtsServing version ${existing_vtsserving_version}"
    fi
else
    pip3 install vtsserving=="$VTSSERVING_VERSION"
fi
"""
            )
            f.write(install_sh)
        ...
        if self.requirements_txt is not None:
            ...
            with vts_fs.open(fs.path.join(py_folder, "requirements.txt"), "w") as f:
                f.write("\n".join(self.packages))
        else:
        ...
        if self.lock_packages and not self.is_empty():
            ...
            pip_compile_in = vts_fs.getsyspath(
                fs.path.combine(py_folder, "requirements.txt")
            )
            pip_compile_out = vts_fs.getsyspath(
...
def _python_options_structure_hook(d: t.Any, _: t.Type[PythonOptions]) -> PythonOptions:
    # Allow vtsfile yaml to have either a str or list of str for these options
...
vtsserving_cattr.register_structure_hook(PythonOptions, _python_options_structure_hook)
...
class VtsBuildConfig:
    ...
    def __attrs_post_init__(self) -> None:
        ...
        if use_cuda and use_conda:
            logger.warning(
                "VtsServing does not support using both conda dependencies and setting a CUDA version for GPU. If you need both conda and CUDA, use a custom base image or create a dockerfile_template, see https://docs.vtsserving.org/en/latest/concepts/vts.html#custom-base-image-advanced"
            )

        if self.docker.distro is not None:
            if use_conda and self.docker.distro not in get_supported_spec("miniconda"):
                raise VtsServingException(
                    f"{self.docker.distro} does not supports conda. VtsServing will only support conda with the following distros: {get_supported_spec('miniconda')}."
                )
    ...
    def from_yaml(cls, stream: t.TextIO) -> VtsBuildConfig:
        ...
        try:
            ...
            if "missing 1 required positional argument: 'service'" in str(e):
                raise InvalidArgument(
                    'Missing required build config field "service", which indicates import path of target vtsserving.Service instance. e.g.: "service: fraud_detector.py:svc"'
                ) from e
            else:
...
class VtsPathSpec:
    ...
    def from_path(self, path: str) -> t.Generator[t.Tuple[str, PathSpec], None, None]:
        """
        yield (parent, exclude_spec) from .vtsignore file of a given path
        """
        fs_ = fs.open_fs(path)
        for file in fs_.walk.files(filter=[".vtsignore"]):
