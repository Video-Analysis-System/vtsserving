...
CLI_LOGGING_CONFIG: dict[str, t.Any] = {
    ...
    "loggers": {
        "vtsserving": {
            "handlers": ["vtsservinghandler", "defaulthandler"],
            "level": logging.INFO,
            "propagate": False,
        },
    },
...
def configure_logging():
    ...
    if get_quiet_mode():
        CLI_LOGGING_CONFIG["loggers"]["vtsserving"]["level"] = logging.ERROR
        CLI_LOGGING_CONFIG["root"]["level"] = logging.ERROR
    elif get_debug_mode():
        CLI_LOGGING_CONFIG["loggers"]["vtsserving"]["level"] = logging.DEBUG
        CLI_LOGGING_CONFIG["root"]["level"] = logging.DEBUG
    else:
        CLI_LOGGING_CONFIG["loggers"]["vtsserving"]["level"] = logging.INFO
...
def trace_record_factory(*args: t.Any, **kwargs: t.Any):
    ...
    if record.name in ("vtsserving_monitor_data", "vtsserving_monitor_schema"):
        return record
    ...
    if trace_id in (0, None):
        ...
        logging_formatting = VtsServingContainer.logging_formatting.get()
...
def configure_server_logging():
    if get_quiet_mode():
        SERVER_LOGGING_CONFIG["loggers"]["vtsserving"]["level"] = logging.ERROR
        SERVER_LOGGING_CONFIG["root"]["level"] = logging.ERROR
    elif get_debug_mode():
        SERVER_LOGGING_CONFIG["loggers"]["vtsserving"]["level"] = logging.DEBUG
        SERVER_LOGGING_CONFIG["root"]["level"] = logging.DEBUG
    else:
        SERVER_LOGGING_CONFIG["loggers"]["vtsserving"]["level"] = logging.INFO
