...
from ...exceptions import VtsServingConfigException
...
def _check_sample_rate(sample_rate: float) -> None:
    if sample_rate == 0.0:
        logger.warning(
            "Tracing enabled, but sample_rate is unset or zero. No traces will be collected. Please refer to https://docs.vtsserving.org/en/latest/guides/tracing.html for more details."
        )
...
@dataclass
class _VtsServingContainerClass:
    ...
    def vtsserving_home() -> str:
        home = expand_env_var(
            str(
                os.environ.get(
                    "VTSSERVING_HOME", os.path.join(os.path.expanduser("~"), "vtsserving")
                )
            )
        )
        vtss = os.path.join(home, "vtss")
        models = os.path.join(home, "models")

        validate_or_create_dir(home, vtss, models)
        return home

    @providers.SingletonFactory
    @staticmethod
    def vts_store_dir(vtsserving_home: str = Provide[vtsserving_home]):
        return os.path.join(vtsserving_home, "vtss")

    @providers.SingletonFactory
    @staticmethod
    def model_store_dir(vtsserving_home: str = Provide[vtsserving_home]):
        return os.path.join(vtsserving_home, "models")
    ...
    def tracer_provider(
        ...
        if (
            ...
            if component_context.vts_name:
                resource[SERVICE_NAMESPACE] = component_context.vts_name
            if component_context.vts_version:
                resource[SERVICE_VERSION] = component_context.vts_version
...
VtsServingContainer = _VtsServingContainerClass()
