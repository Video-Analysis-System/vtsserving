...
class ServiceReloaderPlugin(CircusPlugin):
    ...
    def __init__(self, *args: t.Any, **config: t.Any):
        ...
        self.vtsserving_home = self.config["vtsserving_home"]
        self.working_dir = self.config["working_dir"]

        # a list of folders to watch for changes
        watch_dirs = [self.working_dir, os.path.join(self.vtsserving_home, "models")]

        if not is_pypi_installed_vtsserving():
            # vtsserving src from this __file__
            logger.info(
                "VtsServing is installed via development mode, adding source root to 'watch_dirs'."
            )
            watch_dirs.append(t.cast(str, source_locations("vtsserving")))
