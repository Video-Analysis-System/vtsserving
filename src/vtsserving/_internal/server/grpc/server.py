...
class Server(aio._server.Server):
    ...
    def __init__(
        ...
        | None = Provide[VtsServingContainer.grpc.max_message_length],
        maximum_concurrent_rpcs: int
        | None = Provide[VtsServingContainer.grpc.maximum_concurrent_rpcs],
    ...
    def handlers(self) -> t.Sequence[grpc.GenericRpcHandler] | None:
        # Note that currently VtsServing doesn't provide any specific
        # handlers for gRPC. If users have any specific handlers,
        # VtsServing will pass it through to grpc.aio.Server
        return self.servicer.vts_service.grpc_handlers
    ...
    async def startup(self) -> None:
        ...
        if self.enable_channelz:
            try:
                ...
                raise MissingDependencyException(
                    "'--debug' is passed, which requires 'grpcio-channelz' to be installed. Install with 'pip install vtsserving[grpc-channelz]'."
                ) from None
        ...
        if self.enable_reflection:
            try:
                ...
                raise MissingDependencyException(
                    "reflection is enabled, which requires 'grpcio-reflection' to be installed. Install with 'pip install vtsserving[grpc-reflection]'."
                ) from None
