...
class Servicer:
    ...
    def load(self):
        ...
        self.vts_servicer = create_vts_servicer(self.vts_service)
    ...
    class VtsServiceImpl(services.VtsServiceServicer):
        ...
        async def Call(  # type: ignore (no async types) # pylint: disable=invalid-overridden-method
            ...
            try:
                ...
                await context.abort(
                    code=grpc.StatusCode.INTERNAL,
                    details="An error has occurred in VtsServing user code when handling this request, find the error details in server logs.",
                )
            return response
