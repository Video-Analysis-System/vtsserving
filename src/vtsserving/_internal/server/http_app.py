...
<html lang="en">
  <head>
    <meta charset="UTF-8">
    <title>VtsServing Prediction Service</title>
    <!-- Google Tag Manager -->
  ...
  <body>
    ...
    <div class="version">
        <div class="version-section"><a href="https://github.com/vtsserving/VtsServing" class="github-corner" aria-label="Powered by VtsServing">
...
class HTTPAppFactory(BaseAppFactory):
    ...
    def __init__(
        self,
        vts_service: Service,
        enable_access_control: bool = Provide[VtsServingContainer.http.cors.enabled],
        access_control_options: dict[str, list[str] | str | int] = Provide[
            VtsServingContainer.access_control_options
        ],
    ...
    @property
    def name(self) -> str:
        return self.vts_service.name

    async def index_view_func(self, _: Request) -> Response:
        """
        The default index view for VtsServing API server. This includes the readme
    ...
    async def docs_view_func(self, _: Request) -> Response:
        ...
        return JSONResponse(self.vts_service.openapi_spec.asdict())

    @property
    def routes(self) -> list[BaseRoute]:
        """
        Setup routes for vts model server, including:
        ...
        for _, api in self.vts_service.apis.items():
    ...
    def middlewares(self) -> list[Middleware]:
        ...
        for middleware_cls, options in self.vts_service.middlewares:
        ...
        middlewares.append(
            Middleware(
                ...
                excluded_urls=VtsServingContainer.tracing_excluded_urls.get(),
                default_span_details=None,
                server_request_hook=None,
                client_request_hook=client_request_hook,
                tracer_provider=VtsServingContainer.tracer_provider.get(),
            )
        )

        access_log_config = VtsServingContainer.api_server_config.logging.access
    ...
    def on_startup(self) -> list[t.Callable[[], None]]:
        on_startup = [self.vts_service.on_asgi_app_startup]
    ...
    async def readyz(self, _: "Request") -> "Response":
        if VtsServingContainer.api_server_config.runner_probe.enabled.get():
            runner_statuses = (
                runner.runner_handle_is_ready() for runner in self.vts_service.runners
            )
    ...
    def on_shutdown(self) -> list[t.Callable[[], None]]:
        on_shutdown = [self.vts_service.on_asgi_app_shutdown]
        for runner in self.vts_service.runners:
    ...
    def __call__(self) -> Starlette:
        app = super().__call__()
        for mount_app, path, name in self.vts_service.mount_apps:
        ...
        async def api_func(request: Request) -> Response:
            ...
            try:
                ...
                if trace_context.request_id is not None:
                    response.headers["X-VtsServing-Request-ID"] = str(
                ...
                response = JSONResponse(
                    "An error has occurred in VtsServing user code when handling this request, find the error details in server logs",
                    status_code=500,
                )
            return response
