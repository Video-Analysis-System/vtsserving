...
class HTTPTrafficMetricsMiddleware:
    def __init__(
        self,
        app: ext.ASGIApp,
        namespace: str = "vtsserving_api_server",
    ):
        self.app = app
    ...
    def _setup(
        ...
        duration_buckets: tuple[float, ...] = Provide[
            VtsServingContainer.duration_buckets
        ],
    ):
        self.metrics_client = metrics_client

        DEFAULT_NAMESPACE = "vtsserving_api_server"
        if self.namespace == DEFAULT_NAMESPACE:
            legacy_namespace = "VTSSERVING"
        else:
            legacy_namespace = self.namespace

        # legacy metrics names for vtsserving<1.0.6
        self.legacy_metrics_request_duration = metrics_client.Histogram(
            namespace=legacy_namespace,
            name=metric_name(component_context.vts_name, "request_duration_seconds"),
        ...
        self.legacy_metrics_request_total = metrics_client.Counter(
            namespace=legacy_namespace,
            name=metric_name(component_context.vts_name, "request_total"),
            documentation="Legacy total number of HTTP requests",
            labelnames=["endpoint", "service_version", "http_response_code"],
        )
        self.legacy_metrics_request_in_progress = metrics_client.Gauge(
            namespace=legacy_namespace,
            name=metric_name(component_context.vts_name, "request_in_progress"),
    ...
    async def __call__(
        ...
        with self.legacy_metrics_request_in_progress.labels(
            endpoint=endpoint, service_version=component_context.vts_version
...
class RunnerTrafficMetricsMiddleware:
    def __init__(
        self,
        app: "ext.ASGIApp",
        namespace: str = "vtsserving_runner",
    ):
        self.app = app
        self.namespace = namespace
        self._is_setup = False
