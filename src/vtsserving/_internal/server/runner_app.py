...
from vtsserving.exceptions import ServiceUnavailable
...
feedback_logger = logging.getLogger("vtsserving.feedback")
...
class RunnerAppFactory(BaseAppFactory):
    ...
    def __init__(
        ...
        runner: Runner,
        worker_index: int = 0,
        enable_metrics: bool = Provide[VtsServingContainer.runners_config.metrics.enabled],
    ) -> None:
        self.runner = runner
    ...
    def middlewares(self) -> list[Middleware]:
        ...
        access_log_config = VtsServingContainer.runners_config.logging.access
    ...
    def _mk_request_handler(
        ...
        server_str = f"VtsServing-Runner/{self.runner.name}/{runner_method.name}/{self.worker_index}"
        ...
        async def infer_batch(
            ...
            self.adaptive_batch_size_hist.labels(  # type: ignore
                ...
                service_version=component_context.vts_version,
                service_name=component_context.vts_name,
        ...
        async def _request_handler(request: Request) -> Response:
            ...
            try:
                ...
                return Response(
                    ...
                    headers={
                        PAYLOAD_META_HEADER: "{}",
                        "Content-Type": "application/vnd.vtsserving.error}",
                        "Server": server_str,
                    },
                )
            ...
            if isinstance(payload, Payload):
                return Response(
                    ...
                    headers={
                        ...
                        "Content-Type": f"application/vnd.vtsserving.{payload.container}",
                        "Server": server_str,
                    },
                )
            if isinstance(payload, tuple):
                ...
                return Response(
                    ...
                    headers={
                        PAYLOAD_META_HEADER: json.dumps({}),
                        "Content-Type": "application/vnd.vtsserving.multiple_outputs",
                        "Server": server_str,
                    },
                )
            ...
            try:
                ...
                return Response(
                    ...
                    headers={
                        ...
                        "Server": f"VtsServing-Runner/{self.runner.name}/{runner_method.name}/{self.worker_index}",
                    },
                )
            else:
