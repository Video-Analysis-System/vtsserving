...
class NumpyNdarray(
    IODescriptor["ext.NpNDArray"], descriptor_id="vtsserving.io.NumpyNdarray"
):
    """
    ...
    .. code-block:: python
       ...
       runner = vtsserving.sklearn.get("sklearn_model_clf").to_runner()

       svc = vtsserving.Service("iris-classifier", runners=[runner])
    ...
    Args:
        ...
        shape: Given shape that an array will be converted to. For example:
               ...
               When ``enforce_shape=True`` is provided, VtsServing will raise an exception if the input array received does not match the `shape` provided.

               .. dropdown:: About the behaviour of ``shape``
                  :icon: triangle-down

                  If specified, then both :meth:`vtsserving.io.NumpyNdarray.from_http_request` and :meth:`vtsserving.io.NumpyNdarray.from_proto`
    ...
    Returns:
        :obj:`~vtsserving._internal.io_descriptors.IODescriptor`: IO Descriptor that represents a :code:`np.ndarray`.
    """
