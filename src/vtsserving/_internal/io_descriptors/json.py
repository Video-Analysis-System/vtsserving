...
EXC_MSG = "'pydantic' must be installed to use 'pydantic_model'. Install with 'pip install vtsserving[io-json]'."
...
class DefaultJsonEncoder(json.JSONEncoder):
    def default(self, o: type) -> t.Any:
        ...
        if LazyType["pydantic.BaseModel"]("pydantic.BaseModel").isinstance(o):
            ...
            return obj_dict
        if attr.has(o):
            return vtsserving_cattr.unstructure(o)
        return super().default(o)


class JSON(IODescriptor[JSONType], descriptor_id="vtsserving.io.JSON"):
    """
    ...
    Users then can then serve this service with :code:`vtsserving serve`:
    ...
    Args:
        ...
        json_encoder: JSON encoder class. By default VtsServing implements a custom JSON encoder that
    ...
    def __init__(
        ...
        *,
        ...
        if validate_json is not None:
            logger.warning(
                "'validate_json' option from 'vtsserving.io.JSON' has been deprecated. Use a Pydantic model to specify validation options instead."
            )
    ...
    def from_spec(cls, spec: dict[str, t.Any]) -> Self:
        ...
        if "has_pydantic_model" in spec["args"] and spec["args"]["has_pydantic_model"]:
            logger.warning(
                "VtsServing does not support loading pydantic models from URLs; output will be a normal dictionary."
            )
        if "has_json_encoder" in spec["args"] and spec["args"]["has_json_encoder"]:
            logger.warning(
                "VtsServing does not support loading JSON encoders from URLs; output will be a normal dictionary."
            )
    ...
    def openapi_request_body(self) -> dict[str, t.Any]:
        return {
            ...
            "required": True,
            "x-vtsserving-io-descriptor": self.to_spec(),
        }
