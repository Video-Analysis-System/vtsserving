...
class File(IODescriptor[FileType], descriptor_id="vtsserving.io.File"):
    """
    ...
    .. code-block:: python
       ...
       svc = vtsserving.Service("vit-pdf-classifier", runners=[runner])
    ...
    def _from_sample(self, sample: FileType | str) -> FileType:
        try:
            ...
            raise MissingDependencyException(
                "'filetype' is required to use 'from_sample'. Install it with 'pip install vtsserving[io-file]'."
            )
        ...
        if content_type.decode("utf-8") == "multipart/form-data":
            ...
            for val in form.values():  # type: ignore
                ...
                if len(found_mimes) == 0:
                    raise VtsServingException("no File found in multipart form")
                else:
