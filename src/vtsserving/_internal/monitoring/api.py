...
def monitor(
    ...
    | str
    | None = Provide[VtsServingContainer.config.monitoring.type],
    monitor_options: dict[str, t.Any]
    | None = Provide[VtsServingContainer.config.monitoring.options],
    ...
    :param monitor_class: class of the monitor, can be a string or a class
        ...
        - default
        - otlp
        - "vtsserving.monitoring.prometheus.PrometheusMonitor"
    ...
    Example::
        ...
        with vtsserving.monitor("my_monitor") as m:
    ...
    if name not in MONITOR_REGISTRY:
        if not VtsServingContainer.config.monitoring.enabled.get():
