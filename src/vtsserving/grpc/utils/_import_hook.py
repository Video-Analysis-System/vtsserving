...
def import_generated_stubs(
    ...
    Examples:
        ...
        .. code-block:: python
           ...
           # given proto file vtsserving/grpc/v1/service.proto exists
    ...
    exception_message = f"Generated stubs for '{version}/{file}' are missing (broken installation). Please reinstall vtsserving: 'pip install vtsserving[grpc].'"
    file = file.split(".")[0]

    service_pb2 = LazyLoader(
        f"{file}_pb2",
        globals(),
        f"vtsserving.grpc.{version}.{file}_pb2",
        exc_msg=exception_message,
    )
    service_pb2_grpc = LazyLoader(
        f"{file}_pb2_grpc",
        globals(),
        f"vtsserving.grpc.{version}.{file}_pb2_grpc",
        exc_msg=exception_message,
    )
...
def import_grpc() -> tuple[types.ModuleType, types.ModuleType]:
    ...
    exception_message = "'grpcio' is required for gRPC support. Install with 'pip install vtsserving[grpc]'."
